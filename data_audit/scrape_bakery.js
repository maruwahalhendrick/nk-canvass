// Phase 1: scrape Maps niche BAKERY & KUE + oleh-oleh, list mode
// Output: bakery_candidates.json
...
const OUT = '/home/marwhal/nk-canvass/data_audit/bakery_candidates.json';

const PLAN = [
  { category: 'Bakery & Kue', queries: [
    'toko roti sukabumi',
    'bakery sukabumi',
    'toko kue ulang tahun sukabumi',
    'toko kue basah sukabumi',
    'brownis bolu sukabumi',
    'pastry sukabumi',
      ...
      try {
        ...
        const data = await page.evaluate(() => {
          ...
          feed.querySelectorAll(':scope > div').forEach(b => {
            ...
            const name = t.slice(0, ri).reverse().find(l => !/^(Tempat|Jasa|Toko|Pemasok|Grosir|Distributor|Kedai|Layanan|Kantor|Cabang|Dealer)/.test(l) && l.length > 2) || t[0];
