// Phase 1: scrape Maps niche TOKO JAM (toko jam, arloji, jam tangan, service/ganti baterai)
// Output: jam_candidates.json
...
const OUT = '/home/marwhal/nk-canvass/data_audit/jam_candidates.json';

const PLAN = [
  { category: 'Retail', queries: [
    'toko jam sukabumi',
    'toko arloji sukabumi',
    'jual jam tangan sukabumi',
    'service jam tangan sukabumi',
    'ganti baterai jam sukabumi',
    'jam tangan original sukabumi',
    'toko jam dinding sukabumi',
  ]},
];
      ...
      try {
        ...
        const data = await page.evaluate(() => {
          ...
          feed.querySelectorAll(':scope > div').forEach(b => {
            ...
            const name = t.slice(0, ri).reverse().find(l => !/^(Tempat|Jasa|Toko|Kedai|Layanan|Kantor|Cabang|Dealer|Rental)/.test(l) && l.length > 2) || t[0];
