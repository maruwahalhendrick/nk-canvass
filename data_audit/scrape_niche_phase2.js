...
const CANDS = JSON.parse(fs.readFileSync(process.env.NICHIE_CANDS || '/home/marwhal/nk-canvass/data_audit/gaming_candidates.json', 'utf8'));
const OUT = process.env.NICHIE_OUT || '/home/marwhal/nk-canvass/data_audit/gaming_final.json';
const BASE = JSON.parse(fs.readFileSync(process.env.NICHIE_BASE || '/home/marwhal/nk-canvass/data_audit/final_dataset_full.json', 'utf8'));
const STOP_EXTRA = (process.env.NICHIE_STOP || 'rental|game|games|ps|playstation').split('|');
const STOP = new Set(['sukabumi', 'kota', 'the', 'dan', 'di', 'no', 'cabang', 'terdekat', '24', 'jam', 'toko', 'service', 'servis', 'jasa', ...STOP_EXTRA]);
const OLD = BASE;
...
function sim(a, b) {
  ...
  let hit = 0; ta.forEach(t => { if (tb.some(x => t === x || (t.length >= 4 && x.length >= 4 && (x.includes(t) || t.includes(x))))) hit++; });
