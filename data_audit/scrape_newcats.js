// Phase 1: search Maps per query kategori baru, collect candidates (list mode)
// Output: newcats_candidates.json
...
const OUT = '/home/marwhal/nk-canvass/data_audit/newcats_candidates.json';

// 7 kategori baru, 4 query per kategori
const PLAN = [
  { category: 'Electronics', queries: ['toko aksesoris komputer sukabumi', 'toko hp sukabumi', 'service laptop sukabumi', 'toko elektronik sukabumi'] },
  { category: 'Percetakan', queries: ['percetakan sukabumi', 'digital printing sukabumi', 'sablon kaos sukabumi', 'advertising baliho neon box sukabumi'] },
  { category: 'Fashion', queries: ['distro sukabumi', 'konveksi sukabumi', 'butik sukabumi', 'toko sepatu sukabumi'] },
  { category: 'Agrikultur', queries: ['toko pakan ternak sukabumi', 'toko alat pertanian sukabumi', 'toko obat hewan sukabumi', 'bibit pupuk sukabumi'] },
  { category: 'Olahraga', queries: ['toko sepeda sukabumi', 'gym fitness sukabumi', 'lapangan futsal sukabumi', 'toko pancing sukabumi'] },
  { category: 'Konstruksi', queries: ['toko bangunan sukabumi', 'kontraktor sukabumi', 'toko kaca sukabumi', 'jasa arsitek sukabumi'] },
  { category: 'Keuangan', queries: ['kantor asuransi sukabumi', 'leasing multifinance sukabumi', 'pegadaian sukabumi', 'konsultan pajak sukabumi'] },
];
...
(async () => {
      ...
      try {
        ...
        const data = await page.evaluate(() => {
          ...
          if (h1 && star && !/^hasil$/i.test(h1.innerText.trim())) {
            o.mode = 'panel';
            o.single = { name: h1.innerText.trim(), rating: parseFloat((star.getAttribute('aria-label').match(/([\d.,]+)\s*bintang/) || [])[1]?.replace(',', '.')) };
            return o;
          }
          ...
          feed.querySelectorAll(':scope > div').forEach(b => {
            ...
            const name = t.slice(0, ri).reverse().find(l => !/^(Tempat|Jasa|Toko|Apotek|Bengkel|Kedai|Layanan|Klinik|Restoran|Pemasok|Bengkel|Dealer|Kantor|Cabang|Agent|Agen)/.test(l) && l.length > 2) || t[0];
        ...
        console.log(`[${group.category}] "${q}" -> ${data.mode}${n ? ' (' + n + ')' : ''}`);
      } catch (e) {
        console.log(`[${group.category}] "${q}" ERROR ${e.message.slice(0, 100)}`);
  ...
  await browser.close();
  const total = all.reduce((s, d) => s + (d.candidates ? d.candidates.length : (d.single ? 1 : 0)), 0);
  console.log(`DONE: ${all.length} queries, ${total} raw candidates, captcha=${captchaHit}`);
})();
