// Phase 1: scrape Maps niche CUCI SEPATU (laundry sepatu/deep clean/cuci tas/reparasi), list mode
// Output: shoecare_candidates.json
...
const OUT = '/home/marwhal/nk-canvass/data_audit/shoecare_candidates.json';

const PLAN = [
  { category: 'Cuci Sepatu', queries: [
    'cuci sepatu sukabumi',
    'laundry sepatu sukabumi',
    'deep clean sepatu sukabumi',
    'cuci tas dan sepatu sukabumi',
    'reparasi sepatu sukabumi',
  ]},
];
      ...
      try {
        ...
        const data = await page.evaluate(() => {
          ...
          feed.querySelectorAll(':scope > div').forEach(b => {
            ...
            const name = t.slice(0, ri).reverse().find(l => !/^(Tempat|Jasa|Toko|Apotek|Bengkel|Kedai|Layanan|Klinik|Restoran|Pemasok|Dealer|Kantor|Cabang|Agent|Agen|Rental|Laundry)/.test(l) && l.length > 2) || t[0];
