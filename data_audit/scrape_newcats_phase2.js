// Phase 2: dedupe candidates + visit each unique place for full details (rating, reviews, phone, address, city check)
// Output: newcats_final.json
...
const CANDS = JSON.parse(fs.readFileSync('/home/marwhal/nk-canvass/data_audit/newcats_candidates.json', 'utf8'));
const OUT = '/home/marwhal/nk-canvass/data_audit/newcats_final.json';
const OLD = JSON.parse(fs.readFileSync('/home/marwhal/nk-canvass/data_audit/final_dataset.json', 'utf8'));

const norm = s => String(s).toLowerCase().replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
const STOP = new Set(['sukabumi', 'kota', 'the', 'dan', 'di', 'dan', 'no', 'cabang', 'terdekat', '24', 'jam', 'toko', 'service', 'servis', 'jasa']);
...
(async () => {
  ...
  for (const u of uniques) {
    ...
    try {
      ...
      let info = await page.evaluate(() => {
        ...
        if (feed) {
          ...
          for (const b of blocks) {
            ...
            const name = t.slice(0, ri).reverse().find(l => !/^(Tempat|Jasa|Toko|Bengkel|Kedai|Layanan|Kantor|Dealer|Cabang)/.test(l) && l.length > 2) || t[0];
      ...
      // list mode -> click place link for full panel (phone + address)
    ...
    if (done % 10 === 0) console.log(`[${done}/${uniques.length}] ${Math.round((Date.now() - t0) / 1000)}s`);
