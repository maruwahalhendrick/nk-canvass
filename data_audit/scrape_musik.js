// Phase 1: scrape Maps niche STUDIO MUSIK & SENI (rekaman, latihan band, sekolah musik, sanggar)
// Output: musik_candidates.json
...
const OUT = '/home/marwhal/nk-canvass/data_audit/musik_candidates.json';

const PLAN = [
  { category: 'Studio Musik & Seni', queries: [
    'studio musik sukabumi',
    'studio rekaman sukabumi',
    'tempat latihan band sukabumi',
    'sekolah musik sukabumi',
    'sanggar tari sukabumi',
    'kursus vokal sukabumi',
  ]},
];
      ...
      try {
        ...
        const data = await page.evaluate(() => {
          ...
          feed.querySelectorAll(':scope > div').forEach(b => {
            ...
            const name = t.slice(0, ri).reverse().find(l => !/^(Tempat|Jasa|Toko|Kedai|Layanan|Kantor|Cabang|Dealer|Rental|Sanggar|Sekolah)/.test(l) && l.length > 2) || t[0];
