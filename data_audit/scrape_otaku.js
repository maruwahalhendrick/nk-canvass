// Phase 1: scrape Maps niche OTAKU & COSPLAY (figure, anime, manga, sewa kostum)
// Output: otaku_candidates.json
...
const OUT = '/home/marwhal/nk-canvass/data_audit/otaku_candidates.json';

const PLAN = [
  { category: 'Otaku & Cosplay', queries: [
    'toko action figure sukabumi',
    'toko anime sukabumi',
    'sewa kostum cosplay sukabumi',
    'toko figure gundam sukabumi',
    'toko komik sukabumi',
    'toko nendoroid sukabumi',
  ]},
];
      ...
      try {
        ...
        const data = await page.evaluate(() => {
          ...
          feed.querySelectorAll(':scope > div').forEach(b => {
            ...
            const name = t.slice(0, ri).reverse().find(l => !/^(Tempat|Jasa|Toko|Kedai|Layanan|Kantor|Cabang|Dealer|Kafe|Cafe|Persewaan)/.test(l) && l.length > 2) || t[0];
