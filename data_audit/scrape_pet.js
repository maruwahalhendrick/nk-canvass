// Phase 1: scrape Maps niche PET & HEWAN (pakan, grooming, klinik, aksesoris, ikan hias, burung, aquascape)
// Output: pet_candidates.json
...
const OUT = '/home/marwhal/nk-canvass/data_audit/pet_candidates.json';

const PLAN = [
  { category: 'Pet & Hewan', queries: [
    'toko pakan hewan sukabumi',
    'pet grooming sukabumi',
    'klinik hewan sukabumi',
    'toko aksesoris hewan kucing sukabumi',
    'toko ikan hias sukabumi',
    'toko burung kicau sukabumi',
    'aquascape sukabumi',
  ]},
];
      ...
      try {
        ...
        const data = await page.evaluate(() => {
          ...
          feed.querySelectorAll(':scope > div').forEach(b => {
            ...
            const name = t.slice(0, ri).reverse().find(l => !/^(Tempat|Jasa|Toko|Kedai|Layanan|Kantor|Cabang|Dealer|Rental|Sanggar|Sekolah|Klinik|Dokter)/.test(l) && l.length > 2) || t[0];
