// Phase 1: scrape Maps niche TOKO BANGUNAN/MATERIAL (kategori Konstruksi), list mode
// Output: bangunan_candidates.json
...
const OUT = '/home/marwhal/nk-canvass/data_audit/bangunan_candidates.json';

const PLAN = [
  { category: 'Konstruksi', queries: [
    'toko material bangunan sukabumi',
    'toko semen sukabumi',
    'toko besi beton sukabumi',
    'toko keramik sukabumi',
    'toko cat kayu sukabumi',
    'sanitair keramik sukabumi',
    'toko plywood sukabumi',
    'toko pipa pvc sukabumi',
  ]},
];
      ...
      try {
        ...
        const data = await page.evaluate(() => {
          ...
          feed.querySelectorAll(':scope > div').forEach(b => {
            ...
            const name = t.slice(0, ri).reverse().find(l => !/^(Tempat|Jasa|Toko|Pemasok|Grosir|Distributor|Bengkel|Kedai|Layanan|Kantor|Cabang|Dealer|Rental)/.test(l) && l.length > 2) || t[0];
